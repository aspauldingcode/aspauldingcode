...
export default function ProjectsClient({ projects }: ProjectsClientProps) {
  ...
  const handleSwipe = (card: CardData, direction: 'left' | 'right') => {
    ...
    if (project) {
      ...
      setTimeout(() => {
        ...
        if (direction === 'right') {
          ...
          setSwipedCards(prev => {
            ...
            // Show tutorial hint when user likes their first project
            if (!isAlreadyLiked && !hasShownTutorial && newLikedCards.length === 1) {
      ...
      console.log('❌ Project not found for card ID:', card.id);
    }
  };
  ...
  const handleModalSwipe = (direction: 'left' | 'right') => {
    if (selectedProject) {
      ...
      // If opened from liked section, update the liked/dismissed state
