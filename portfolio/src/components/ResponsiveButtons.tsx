...
const ResponsiveButton: React.FC<ResponsiveButtonProps> = ({
  ...
  useEffect(() => {
    const checkTextFit = () => {
      ...
      const buttonPadding = 24; // Account for padding (0.75rem * 2)
      ...
      measureSpan.style.fontSize = window.getComputedStyle(button).fontSize;
      measureSpan.style.fontFamily = window.getComputedStyle(button).fontFamily;
      measureSpan.style.fontWeight = window.getComputedStyle(button).fontWeight;
      document.body.appendChild(measureSpan);

      // Test each text level to find the one that fits
      const levels: Array<{ key: 'full' | 'medium' | 'short' | 'icon'; text: string }> = [
        { key: 'full', text: texts.full },
        { key: 'medium', text: texts.medium },
        { key: 'short', text: texts.short },
        { key: 'icon', text: '' } // Icon doesn't need text measurement
      ];

      let selectedLevel: 'full' | 'medium' | 'short' | 'icon' = 'icon';
      ...
      for (const level of levels) {
        ...
        measureSpan.textContent = level.text;
